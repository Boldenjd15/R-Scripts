#Football Data
football <- read.table("C:/Users/dedwards7/Desktop/Teaching/DAPT 622/R files/football.txt",sep=',',header=TRUE)
...
#Bank Data
library(MASS)
bank.data <- read.table("C:/Users/dedwards7/Desktop/Teaching/DAPT 622/R files/bank_data.txt",sep=',',header=TRUE)
...
vars <- bank_sub[,-8]
pooled <- pooled.cov(as.matrix(vars),bank_sub$target)
sds <- diag(pooled)
std.coef <- bank_lda$scaling*sds
